...
export default function ScoreEntryMask() {
  ...
  async function loadScores(playerId: string, round: number) {
    if (!playerId) {
      setScores({});
      return;
    }
    ...
    const { data, error } = await supabase
      .from("scores")
      .select(
        "registration_id,hole_number,strokes,tournament_id,round_number"
      )
      .eq("tournament_id", TOURNAMENT_ID)
    ...
    if (error) {
      ...
      ((data ?? []) as ScoreRow[]).forEach((row) => {
        next[row.hole_number] =
          row.strokes == null ? "" : String(row.strokes);
      });
  ...
  useEffect(() => {
    loadScores(selectedPlayerId, selectedRound);
  ...
  async function saveScores() {
    ...
    const { error } = await supabase
      ...
      .upsert(rows, {
        onConflict:
          "registration_id,tournament_id,round_number,hole_number",
      });
    ...
    if (error) {
      ...
      setMessage(
        `Scores für Runde ${selectedRound} erfolgreich gespeichert.`
      );
      ...
      <div className="mx-auto max-w-5xl space-y-6">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div>
              ...
              <h1 className="mt-1 text-3xl font-black text-slate-900">
                Manuelle Scoreeingabe
              </h1>

              <p className="mt-2 text-sm text-slate-600">
                Spieler suchen, Runde auswählen und Scores direkt in das Live-Leaderboard eintragen.
              </p>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {selectedPlayer
                    ? `${fullName(selectedPlayer)} · Runde ${selectedRound}`
                    : "Noch kein Spieler ausgewählt"}
                </p>
              </div>
              ...
              {Array.from(
                ...
                >
                  ...
                  <input
                    ...
                    placeholder="-"
                    className="w-full rounded-xl border border-slate-200 px-2 py-3 text-center text-xl font-black outline-none"
                  />
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Die Scores werden direkt in die Daten des Live-Leaderboards geschrieben.
            </p>
            ...
            >
              ...
              {saving
                ? "Speichert ..."
                : `Runde ${selectedRound} speichern`}
            </button>

            {message ? (
              <div className="mt-4 inline-flex w-full items-start gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
